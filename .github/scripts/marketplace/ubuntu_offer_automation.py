...
def run_ubuntu_automation():
    ...
    # --- UBUNTU CONSOLIDATED MAPPING TOPOLOGY ---
    product_alias = "ubuntu-server-gen2"
    
    ubuntu_plans = [
        ("plan-ubuntu-2404", "Ubuntu Server 24.04 LTS all plans"),
        ("plan-ubuntu-2404-minimal", "Ubuntu Server 24.04 Minimal"),
        ("plan-ubuntu-2204", "Ubuntu Server 22.04 LTS all plans"),
        ("plan-ubuntu-2204-minimal", "Ubuntu Server 22.04 Minimal"),
        ("plan-ubuntu-2004", "Ubuntu Server 20.04 LTS all plans")
    ]
    
    resources = []
    
    print(f"Synthesizing the Unified Ubuntu Server Root Offer ({product_alias})...")
    ...
    # 2. Plan Minting Loop
    for p_id, name_str in ubuntu_plans:
        ...
        resources.append({
            ...
            "name": name_str,
            "summary": name_str,
            "description": f"<h2>{name_str}</h2><p>Experience enterprise-grade security and reliability with {name_str}. Natively built by Canonical for peak performance on Azure Gen2 hypervisor architectures.</p>"
        })
        
        # Hardware & Image Technical Configuration
        # Dynamically map the p_id alias back to the Packer target (e.g. 'plan-ubuntu-2404' -> 'imgdef-ubuntu-server-2404-gen2')
    ...
    print(f"\nExecuting POST /configure to mint '{product_alias}' and {len(ubuntu_plans)} Nested Plans...")
    try:
